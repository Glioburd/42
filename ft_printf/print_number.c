...
/*   Updated: 2015/02/05 17:07:14 by gsauvair         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ul(int flag, char *base, va_list ap)
{
	...
	if (flag == 5)
		str = ultoa(base, va_arg(ap, unsigned long));
	else
		str = litoa(base, va_arg(ap, long int));
	return (str);
}
...
char	*ft_print_number(t_opt opt, va_list ap, int flag)
{
	...
	else
	{
		if (flag == 4 || (flag >= 6 && flag <= 8))
			if (flag == 8)
				str = ft_strjoin("0x", ultoa(base, va_arg(ap, long)));
			else
			{
				n = va_arg(ap, int);
				str = uitoa(base, n);
			...
			str = ultoa(base, va_arg(ap, unsigned int));
		else
			str = itoa(base, va_arg(ap, int));
	}
